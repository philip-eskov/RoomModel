...
class RoomModel:
    ...
    def __init__(
        ...
        self.__impedance_val_array = np.full(
            (self.n_space, 2), (self.air_z0, self.air_speed)
        )
    ...
    def add_impedant_object(
        self, start: float, stop: float, char_impedance: float, density: float
    ):
    ...
    def calc_intensity(self):
        ...
        for i in range(len(imp_arr) - 1):
            ...
            wave_anal_sol[i, :] += max_amplitude * np.sin(
                wavenumber * self.x_arr[i] - self.omega * self.t_arr
            )
        ...
        ani = animation.FuncAnimation(
            fig, myanimation, frames=len(self.t_arr), interval=100
        )
